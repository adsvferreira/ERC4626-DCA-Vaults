...
from brownie import TreasuryVault, exceptions, reverts
from helpers import get_account_from_pk, check_network_is_mainnet_fork, encode_custom_error
...
def test_withdraw_native_by_non_owner():
    ...
    treasury_vault = TreasuryVault[-1]
    # Act / Assert
    with reverts(encode_custom_error(TreasuryVault, "OwnableUnauthorizedAccount", []) + abi.encode(["address"], [dev_wallet2.address]).hex()):
...
def test_withdraw_native_amount_gt_total_balance_by_owner():
    ...
    # Act / Assert
    with reverts(encode_custom_error(TreasuryVault, "NotEnoughEther", []) + abi.encode(["string"], ["Insufficient balance"]).hex()):
...
def test_withdraw_erc20_amount_gt_total_balance_by_owner(deposit_token):
    ...
    # Act / Assert
    with reverts(encode_custom_error(TreasuryVault, "InvalidTokenBalance", []) + abi.encode(["string"], ["InvalidTokenBalance"]).hex()):
