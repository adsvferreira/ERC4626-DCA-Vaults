...
class StrategyFetcher:
    ...
    def fetch_vaults(
        ...
        if (
            buy_frequency_timestamp
            and buy_frequency_timestamp
            not in buy_frequency_enum_to_seconds_map.values()
        ):
            ...
            for vault_address in vault_addresses:
                ...
                vault_buy_frequency_timestamp = (
                    self.__get_vault_buy_frequency_timestamp(strategy_params)
                )
                if (
                    buy_frequency_timestamp
                    and buy_frequency_timestamp != vault_buy_frequency_timestamp
                ):
                    continue
                ...
                vault = StrategyVault(
                    ...
                    tokens_to_buy_decimals=self.__get_token_addresses_to_buy_decimals(
                        vault_contract, token_addresses_to_buy_length
                    ),
                    buy_amounts=self.__get_buy_amounts(
                        strategy_params, token_addresses_to_buy_length
                    ),
                    depositor_addresses=self.__get_depositor_addresses(
                        vault_contract, all_depositors_length
                    ),
    ...
    def __get_token_addresses_to_buy(
        ...
        return [
            vault_contract.buyAssetAddresses(i)
            for i in range(token_addresses_to_buy_length)
        ]

    def __get_token_addresses_to_buy_decimals(
        self, vault_contract: AutomatedVaultERC4626, token_addresses_to_buy_length: int
    ) -> List[int]:
        return [
            vault_contract.buyAssetsDecimals(i)
            for i in range(token_addresses_to_buy_length)
        ]

    def __get_buy_amounts(
        self, strategy_params: tuple, token_addresses_to_buy_length: int
    ) -> List[int]:
        return [strategy_params[0][i] for i in range(token_addresses_to_buy_length)]

    def __get_depositor_addresses(
        self, vault_contract: AutomatedVaultERC4626, all_depositors_length: int
    ) -> List[str]:
        return [
            vault_contract.allDepositorAddresses(i)
            for i in range(all_depositors_length)
        ]
