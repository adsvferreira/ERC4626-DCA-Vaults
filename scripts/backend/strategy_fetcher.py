...
class StrategyFetcher:
    ...
    def fetch_vaults(
        self, vault_addresses: List[str], buy_frequency_timestamp: Union[int, None] = None
    ) -> List[StrategyVault]:
        if buy_frequency_timestamp and buy_frequency_timestamp not in buy_frequency_enum_to_seconds_map.values():
            ...
            for vault_address in vault_addresses:
                ...
                vault = StrategyVault(
                    ...
                    tokens_to_buy_decimals=self.__get_token_addresses_to_buy_decimals(vault_contract, token_addresses_to_buy_length),
                    buy_amounts=self.__get_buy_amounts(strategy_params, token_addresses_to_buy_length),
    ...
    def __get_token_addresses_to_buy_decimals(
        ...
        return [vault_contract.buyAssetsDecimals(i) for i in range(token_addresses_to_buy_length)]

    def __get_buy_amounts(self, strategy_params: tuple, token_addresses_to_buy_length: int) -> List[int]:
