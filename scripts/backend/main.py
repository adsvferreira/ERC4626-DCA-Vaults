...
def main():
    ...
    all_vaults = [
        ...
        if vault.address
        not in [
            "0x797785F3EaEB02aC140219cC0cfFDcF597aAFDF3",
            "0xfbD1a54CaB7eFeD75dB500fd10a92Fa91686A5B3",
            "0x77a2Ca16c0414B1FAb6c1EED3286340A1aaCe06F",
            "0xCab54a8FBC9600F5b9a0f53912946eEEa5980d9f",
        ]
    ]
    ...
    while True:
        ...
        for vault in all_vaults:
            ...
            print(
                f"Vault {vault.address} last updated timestamp: {vault.last_update_timestamp}"
            )
            if (
                current_time - vault.last_update_timestamp
                >= vault.buy_frequency_timestamp
            ):
                for depositor_address in vault.depositor_addresses:
                    try:
                        tx = controller_executor.trigger_strategy_action(
                            vault.address, depositor_address
                        )
                        tx.wait(1)
                        print(
                            f"WALLET: {depositor_address} BALANCES SWAPPED AND SENT TO DESTINATION WALLET"
                        )
                    except Exception:
                        print(f"TRANSACTION FAILED FOR WALLET: {depositor_address}")
                print("VAULT DETAILS:")
